...
class WakeWordDetector:
    ...
    def _init_detector_with_fallback(
        ...
        try:
            ...
            if (
                "inference_framework" in str(exc)
                and "inference_framework" in model_kwargs
            ):
                model_kwargs_copy = dict(model_kwargs)
                model_kwargs_copy.pop("inference_framework")
                try:
                    return model_cls(**model_kwargs_copy)
                except Exception:
                    # ignore inner exception, continue with fallback logic
                    pass
            else:
                raise
        except Exception:
        ...
        if (
            model_kwargs.get("inference_framework") != "onnx"
            and "inference_framework" in model_kwargs
        ):
            raise
        ...
        logger.warning(
            ...
            f"{[Path(p).name for p in available_models]}"
        )
        model_kwargs["wakeword_models"] = available_models
        # In case it doesn't like inference_framework when loading discovered models either
        if "inference_framework" in model_kwargs:
            try:
                return model_cls(**model_kwargs)
            except TypeError:
                model_kwargs.pop("inference_framework")
                return model_cls(**model_kwargs)
        return model_cls(**model_kwargs)
