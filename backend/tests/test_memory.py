...
@pytest.mark.asyncio
async def test_long_term_store_and_search(tmp_path) -> None:
    from storage import db as db_module

    db_module.db = Database(db_path=tmp_path / "memory.db")
    await db_module.db.initialize()

    long_term = LongTermMemory()
    await long_term.store_summary("s1", "User asked about python and vscode", ["python", "vscode"], turn_count=4)
...
async def test_context_builder() -> None:
    ...
    builder = ContextBuilder(short_term_memory=short_term, long_term_memory=DummyLongTerm(), token_budget=4000)
    messages = await builder.build_context("Open terminal")
    assert messages[0]["role"] == "system"
    assert any(m["role"] == "user" and "Open terminal" in m["content"] for m in messages)
