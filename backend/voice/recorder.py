...
class AudioRecorder:
    ...
    async def start_recording(self) -> RecordingResult | None:
        self._cancel.clear()

        try:
            import sounddevice as sd
        except Exception as exc:  # pragma: no cover - runtime dependency
            await self._event_bus.publish("listener_error", {"message": f"sounddevice unavailable: {exc}"})
            return None

        try:
            import webrtcvad
        except Exception as exc:  # pragma: no cover - runtime dependency
            await self._event_bus.publish("listener_error", {"message": f"webrtcvad unavailable: {exc}"})
            return None
                ...
                for _ in range(max_frames):
                    ...
                    if (now - last_level_emit) >= 0.05:
                        ...
                        if normalized.size >= bins:
                            ...
                            levels = [
                                float(np.mean(normalized[index * window : (index + 1) * window]))
                    ...
                    frames.append(mono_chunk.astype(np.int16))
                    ...
                    if not detected_speech and no_speech_frames >= no_speech_frames_limit:
                        await self._event_bus.publish("recording_timeout", {"reason": "no_speech"})
                        return None
